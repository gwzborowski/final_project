...
Callbacks defined here:
  ...
  2. update_search_results  -- location + radius + mood button click
                                -> geocode -> Overpass query -> map + list + stores
  3. update_weather_nudge   -- location-store changes -> Open-Meteo call -> mood nudge text
"""
...
from utils.data_sources import (
    ...
    geocode_location,
    get_weather,
    mood_nudge_from_weather,
    search_locations,
)

dash.register_page(__name__, path="/search", name="Search", order=0)
    ...
    [
        html.Div(
            ...
            children=[
                ...
                html.P(
                    "Type a city or address, set a radius, and optionally filter by mood."
                ),

                html.Label(
                    "Location",
                    className="control-label label-pink"
                ),
...
# ---------------------------------------------------------------------------
# Plotly map compatibility
# ---------------------------------------------------------------------------

# Plotly renamed/replaced the Mapbox-token-based trace
# (go.Scattermapbox / layout.mapbox) with a token-free MapLibre trace
# (go.Scattermap / layout.map) starting in plotly 5.24.
...
# Default view when nothing has been searched yet.
    ...
    fig.update_layout(
        **{
            _MAP_LAYOUT_KEY: dict(
                ...
                center=center or _DEFAULT_CENTER,
                zoom=zoom or _DEFAULT_ZOOM,
            )
        },
...
def _shops_to_map(df, center_lat, center_lon):
    ...
    the average of whatever shops came back.

    Each shop category is plotted as its own trace with a distinct emoji icon.
    """
    ...
    # Marker for searched location itself
    traces.append(
        _MAP_TRACE(
            lat=[center_lat],
            lon=[center_lon],
...
def _shops_to_cards(df):
    if df.empty:
        return html.P(
            "No shops matched -- try widening the radius "
            "or clearing the mood filter."
        )
            ...
            html.Div(
                ...
                children=[
                    ...
                    html.P(
                        f"{row['category']} · "
                        f"mood: {row['mood_tag']} · "
                        f"{row.get('price_level', 'N/A')} · "
                        f"⭐ {row.get('rating', 'N/A')}"
                    ),
                    html.P(
                        row["address"] or "Address unavailable",
                        style={
                            "fontSize": "13px",
                            "color": "#6b5245",
                        },
...
# ---------------------------------------------------------------------------
# Callback 1 -- live typing in Location dropdown -> geocoding suggestions
# ---------------------------------------------------------------------------
...
def update_location_options(search_value, current_value):
    if not search_value:
        return dash.no_update

    matches = search_locations(search_value)
    ...
    # Keep the currently-selected value in the list so it doesn't disappear
    # from the dropdown label if it isn't among the latest search matches.
...
# ---------------------------------------------------------------------------
# Callback 2 -- Location + mood input -> shop results
# ---------------------------------------------------------------------------
...
def update_search_results(
    ...
    if not n_clicks:
        return (
            ...
            _empty_map(),
            html.P(
                "Enter a location above and click Search to get started!"
            ),
            "",
        )
    ...
    if lat is None:
        return (
            ...
            dash.no_update,
            dash.no_update,
            f"Couldn't find '{location_text}'. "
            "Try a city and state, e.g. 'Norfolk, VA'.",
        )

    # Round coordinates before lookup so the cache hits on repeat searches.
    df, source, error_message = find_shops(
        round(lat, 3),
        round(lon, 3),
        radius_miles or 5,
    )
    ...
    if df.empty:
        ...
        source_label = {
            "yelp": "via Yelp",
            "overpass": "via OpenStreetMap",
        }.get(source, "")

        if source_label:
            status = (
                f"Showing {len(df)} spot(s) near "
                f"{display_name} ({source_label})."
            )
        else:
            status = (
                f"Showing {len(df)} spot(s) near "
    ...
    # Map always centers on the searched point.
    fig = _shops_to_map(
        df,
        lat,
        lon,
...
# ---------------------------------------------------------------------------
# Callback 3 -- lat/lon -> live weather call -> mood nudge
# ---------------------------------------------------------------------------
...
def update_weather_nudge(location_data):
    ...
    # Pass the location name into the mood nudge function.
    message, _mood_tag = mood_nudge_from_weather(
        weather,
